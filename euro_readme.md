...
## Per-Individual Tiled BED with Gene-Flank Counts

A second large per-individual tiled BED was generated by combining:

- `jri_v5_flanks.bed`
- `v5_100kb_tiles_flanks.bed`

### Inputs used

#### `jri_v5_flanks.bed`

This file contains the lifted crossover intervals from `jri_v5.bed` with appended flank-overlap counts:
...
- individual identity
- interval ID
- number of overlapping 5' upstream regions
- number of overlapping 3' downstream regions

#### `v5_100kb_tiles_flanks.bed`

This file contains the 100 kb genome tiles with appended flank-overlap counts:
...
For each individual in column 4 of `jri_v5_flanks.bed`, construct a genome-wide interval track that contains:

- the individual's original `jri_v5_flanks.bed` intervals
- all 100 kb tiles from `v5_100kb_tiles_flanks.bed` that do not overlap any of that individual's `jri_v5_flanks.bed` intervals
...
- each original interval from `jri_v5_flanks.bed` was kept as-is
- each 100 kb tile from `v5_100kb_tiles_flanks.bed` was tested against that individual's `jri_v5_flanks.bed` intervals
- if a tile overlapped any `jri_v5_flanks.bed` interval for that individual, the tile was dropped
- if a tile did not overlap any `jri_v5_flanks.bed` interval for that individual, the tile was retained

This means the final track for each individual contains no overlaps between:

- original `jri_v5_flanks.bed` intervals
- retained 100 kb tiles

### Output file

The concatenated output file is:

- `results/jri_v5_flanks_with_tiles.bed`

### Output columns

This BED has 7 columns:

- column 1: chromosome
- column 2: start
- column 3: end
- column 4: individual identity
- column 5: number of overlapping 5' flanking regions
- column 6: number of overlapping 3' flanking regions
- column 7: source flag

Interpretation of column 7:

- `1` = interval came from `jri_v5_flanks.bed`
- `0` = interval came from `v5_100kb_tiles_flanks.bed`

### Script used

This file was generated with:

- `scripts/build_individual_flank_tiled_track.py`

### Size of the final file

For the current run:

- output file: `results/jri_v5_flanks_with_tiles.bed`
- size: `10,491,403,942` bytes

This file is intended as a per-individual genome annotation track with appended gene-flank overlap counts.
